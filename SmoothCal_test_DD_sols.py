...
def DD_StefCal(Vpq, Wpq, Xpq, gains, Na, Nnu, Nt, Nsource, A, Sigmayinv, j, maxiter=100, tol=1e-3):
    diff = 1.0
    i = 0
    while i<maxiter and diff >= tol:
        gold = gains.copy()
        A[...] = give_response(Xpq, gold, Na, Nnu, Nt, Nsource, A)
...
if __name__ == "__main__":
    ...
    Nsource = 3
    max_I = 2.0
    IM, lm, locs, alphas = Simulator.sim_sky(Npix, Nsource, max_I, lmax, mmax, nu, nu[Nnu//2])
    ...
    Xpq_DD = np.ma.MaskedArray(Xpq_DD, I_DD)
    ...
    # test ML solution
    gains = gains.swapaxes(2, 3)  # we need time on the last axis
    gbar = np.ones_like(gains, dtype=np.complex128)
    gbar[...] = DD_StefCal(Vpq, Wpq, Xpq_DD, gbar, Na, Nnu, Nt, Nsource, A, Sigmayinv, j, tol=5e-3, maxiter=20)

    print np.abs(gbar - gains).max()
